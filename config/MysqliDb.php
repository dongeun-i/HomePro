...
class MysqliDb
{
    ...
    {
        $res = $this->ArrayBuilder()
                    ->get($tableName, $limit, "{$column} AS retval");
        if (!$res) {
            return null;
        }
        if ($limit == 1) {
    ...
    public function insert($tableName, $insertData)
    {
        ...
        return $this->_buildInsert($tableName, $data, 'INSERT');
    }
    ...
    public function update($tableName, $tableData, $numRows = null)
    {
        ...
        $stmt = $this->_buildQuery($numRows, $data);
        $status = $stmt->execute();
        $this->reset();
        $this->_stmtError = $stmt->error;
        $this->_stmtErrno = $stmt->errno;

        if (count($stmt->error_list) > 0 || $stmt->error || $stmt->errno) {
            $this->rollback();
            throw new Exception
            ('error_list: ' .
