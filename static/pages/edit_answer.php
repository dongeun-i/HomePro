...
<?php
    if(isset($_GET['id'])){
        $qa = Qa::selectOne($_GET['id']);
        //print_r($qa);
    }else{
        echo "<script>alert('올바르지 못한 접근입니다.');self.close();</script>";
    }
...
?>
    ...
    <form action="process/qa_proc.php" method="post" autocomplete="off">
        ...
        <input type="text" id="name" name="name" autocomplete="off" value="<?= $qa['name']?>" readonly>

        <label for="title" class="require">제목</label>
        <input type="text" id="title" name="title" value="<?=$qa['title']?>" required autocomplete="off" readonly>

        <label for="content" class="require">내용</label>
        <textarea id="content" name="content" rows="5" required autocomplete="off" readonly><?=$qa['question']?></textarea>

        <label for="content" class="require">답변</label>
        <textarea id="answer" name="answer" rows="5" required autocomplete="off"></textarea>
